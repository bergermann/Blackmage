...
function mcTarget(device_mc::TCPSocket,device_ids::TCPSocket,target::Real,unit::Symbol,
    ...
    for addr in 1:3
        mcTargetP(device_mc,device_ids,addr,dz[addr]*units[unit],:m; maxsteps=100,maxiter=10)
    end

    for addr in 1:3
        mcTargetP(device_mc,device_ids,addr,dz[addr]*units[unit],:m; maxsteps=10,maxiter=10)
    end
